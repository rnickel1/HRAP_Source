...
class ChemSolver:
    ...
    def __init__(self, chem_path):
        ...
        """
        with chem_file as open(chem_path, 'r'):
            strip = ' '
                ...
                if line.startswith('thermo'):
                    ...
                    if fitPieces == 0: # If single temp (only appropriate for condensed reactants)
                        ...
                        for j in range(fitPieces):
                            ...
                            ncoffs = std::stoi(line.substr(i, i+1).strip(strip)); i += 1
                            if ncoffs > 8:
                    ...
                    self.substances[formula] = ThermoSubstance(formula, comment, condensed, not in_reactants, composition, M, providers, T_min, T_max)
