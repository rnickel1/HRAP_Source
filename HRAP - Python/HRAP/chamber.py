...
def d_chamber(s, x, xmap):
    ...
    V        = s['cmbr_V'] - x[xmap['grn_V']] # Gas volume in chamber
    dV       = s['cmbr_V'] - x[xmap['grn_Vdot']] # Gas volume in chamber derivative
    OF       = x[xmap['cmbr_OF']] # O/F ratio, set by grain file

    # Chamber stored mass derivative
    mdot_g = cond(m_g <= 0.0, lambda val: 0.0, lambda val: val, grn_mdot + inj_mdot - noz_mdot)

    # Chamber pressure derivative
    Pdot = cond(Pc <= s['Pa'], lambda val: 0.0, lambda val: val, Pc*(mdot_g/m_g - dV/V))
    ...
    # Store derivatives
    x = store_x(x, xmap, cmbr_mdot_g=mdot_g, cmbr_P=Pdot, cmbr_k=k, cmbr_cstar=cstar)
